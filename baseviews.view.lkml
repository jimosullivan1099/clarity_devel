...
explore: client {
  ...
  join: enrollments {
    ...
    sql_on: ${client.id} = ${enrollments.ref_client} and CASE WHEN '{{ _access_filters["agencies.id"]] }}'  = '>0'  THEN ${enrollments.ref_agency} >0 ELSE ${enrollments.ref_agency} < 0 END and ${enrollments.deleted} is null ;;
  }
